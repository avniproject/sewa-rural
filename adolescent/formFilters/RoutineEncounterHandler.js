...
export default class RoutineEncounterHandler {
    ...
    schoolGoing(programEncounter, formElement) {
        const statusBuilder = this._getStatusBuilder(
            programEncounter,
            formElement,
        ...
        statusBuilder
            .show()
            .whenItem(this._isFirstAnnualVisit(programEncounter))
            .equals(false);
    ...
    nameOfSchool(programEncounter, formElement) {
        ...
        statusBuilder
            ...
            .and.when.latestValueInEntireEnrolment("School going")
    ...
    inWhichStandardHeSheIsStudying(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.latestValueInEntireEnrolment("School going")
            .containsAnswerConceptName("Yes")
    ...
    rollNumber(programEncounter, formElement) {
        return this.inWhichStandardHeSheIsStudying(programEncounter, formElement).and(
            this._registeredAtSchoolOrBoarding(
                programEncounter,
                formElement,
                RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE
            )
    ...
    parents(programEncounter, formElement) {
        return this._hasBeenComingToSchool(programEncounter, formElement, RoutineEncounterHandler.visits.ANNUAL)
            ...
            .and(
                this._notEleventhTwelfthAdolescentRegisteredAtSchoolOrBoardingSchool(
                    programEncounter,
                    formElement,
                    RoutineEncounterHandler.visits.ANNUAL
                )
    ...
    stayingWithWhom(programEncounter, formElement) {
        let statusBuilder = this._getStatusBuilder(
            programEncounter,
            formElement,
            RoutineEncounterHandler.visits.ANNUAL
        );
        ...
        statusBuilder
            ...
            .containsAnswerConceptName("Both Expired");
    ...
    height(programEncounter, formElement) {
        return this._getStatusBuilder(
            programEncounter,
            formElement,
            RoutineEncounterHandler.visits.HALF_YEARLY_OR_MIDLINE
        )
            .build()
            .and(
                this._notDroppedOutOrRegisteredAtVillage(
                    programEncounter,
                    formElement,
                    RoutineEncounterHandler.visits.HALF_YEARLY_OR_MIDLINE
                )
    ...
    bmi(programEncounter, formElement) {
        ...
        let formElmentStatus = this._notDroppedOutOrRegisteredAtVillage(
            programEncounter,
            formElement,
            RoutineEncounterHandler.visits.HALF_YEARLY_OR_MIDLINE
        ).and(
    ...
    hemoglobinTestDone(programEncounter, formElement) {
        return this._getStatusBuilder(programEncounter, formElement, RoutineEncounterHandler.visits.ANNUAL_OR_MIDLINE)
            .build()
            .and(
                this._notDroppedOutOrRegisteredAtVillage(
                    programEncounter,
                    formElement,
                    RoutineEncounterHandler.visits.ANNUAL_OR_MIDLINE
                )
            )
            .and(
                this._notEleventhTwelfthAdolescentRegisteredAtSchoolOrBoardingSchool(
    ...
    hemoglobinTest(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.valueInEncounter("Hemoglobin Test Done")
            .containsAnswerConceptName("Yes");
        return statusBuilder.build();
    }

    sicklingTestDone(programEncounter, formElement) {
        return this._getStatusBuilder(programEncounter, formElement, RoutineEncounterHandler.visits.ANNUAL)
            .build()
            .and(
                this._notDroppedOutOrRegisteredAtVillage(
                    programEncounter,
    ...
    sicklingTestResult(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.valueInEncounter("Sickling Test Done")
            .containsAnswerConceptName("Yes");
        return statusBuilder.build();
    }
    ...
    areYouTakingRegularTreatmentForSickleCellDisease(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.latestValueInEntireEnrolment("Are you taking regular treatment for Sickle cell disease?")
    ...
    ironTablets(programEncounter, formElement) {
        return this._getStatusBuilder(programEncounter, formElement, RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE)
            .build()
            .and(
                this._notDroppedOutOrRegisteredAtVillage(
                    programEncounter,
    ...
    albendazoleTabletsReceived(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .whenItem(
                new RuleCondition({programEncounter: programEncounter}).when.encounterMonth
                    .equals(9)
                    .or.equals(3)
                    .matches()
            )
            .is.truthy.and.whenItem(
                new RuleCondition({programEncounter: programEncounter}).when
                    .latestValueInAllEncounters("Standard")
                    .not.containsAnyAnswerConceptName("11", "12")
                    .matches()
            ).is.truthy;
    ...
    menstruationStarted(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.female.and.latestValueInPreviousEncounters("Menstruation started")
            .not.containsAnswerConceptName("Yes");
    ...
    otherReasonPleaseSpecify(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.valueInEncounter("Reason for remaining absent during menstruation")
    ...
    mhmKitReceived(programEncounter, formElement) {
        ...
        let afterDate = moment(programEncounter.encounterDateTime)
            .subtract(6, "months")
            .toDate();
        let mhmKitReceivedStatus = programEncounter.programEnrolment.hasEncounterWithObservationValueAfterDate(
            "Monthly Visit",
            afterDate,
            "MHM Kit received",
            "Yes"
        ...
        statusBuilder
            .show()
            .latestValueInAllEncounters("Menstruation started")
            .containsAnswerConceptName("Yes")
    ...
    mhmKitUsed(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.latestValueInAllEncounters("MHM Kit received")
            .containsAnswerConceptName("Yes");
        return statusBuilder.build();
    }
    ...
    sicknessInLast3Months(programEncounter, formElement) {
        return this._getStatusBuilder(
            programEncounter,
            formElement,
            RoutineEncounterHandler.visits.QUARTERLY_OR_MIDLINE
        )
            .build()
            .and(
                this._notDroppedOutOrRegisteredAtVillage(
    ...
    otherSicknessPleaseSpecify(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.valueInEncounter("Sickness in last 3 months")
            .containsAnswerConceptName("Other");
    ...
    areYouSexuallyActive(programEncounter, formElement) {
        return this._getStatusBuilder(programEncounter, formElement, RoutineEncounterHandler.visits.HALF_YEARLY)
            .build()
            .and(
                this._notDroppedOutOrRegisteredAtVillage(
                    programEncounter,
                    formElement,
                    RoutineEncounterHandler.visits.HALF_YEARLY
                )
            )
            .and(
                this._notEleventhTwelfthAdolescentRegisteredAtSchoolOrBoardingSchool(
    ...
    doYouHaveLicenceForTheVehicle(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.valueInEncounter("Drives 2 wheeler")
            .containsAnswerConceptName("Yes")
            .and.whenItem(this._isAdolescentEleventhTwelfthStandardAndRegisteredAtSchoolOrBoarding(programEncounter))
            .equals(false);
    ...
    howTheAccidentHasHappened(programEncounter, formElement) {
        ...
        statusBuilder
            .show()
            .when.valueInEncounter("Road accident in past 6 months")
            .containsAnswerConceptName("Yes");
    ...
    counsellingChecklistForMenstrualDisorder(programEncounter, formElement) {
        return new FormElementStatus(
            formElement.uuid,
            this._applicableForMenstrualDisorderCounselling(programEncounter)
        );
    }
    ...
    counsellingChecklistForSickleCellAnemiaTrait(programEncounter, formElement) {
        return new FormElementStatus(
            formElement.uuid,
            this._applicableForSickleCellAnemiaCounselling(programEncounter)
        );
    }
    ...
    whichOfTheFollowingAilmentsDidYouVisitTheHospitalFor(programEncounter, formElement) {
        ...
        const allAnswerConcepts = formElement.concept.answers.map(answer =>
            Object.assign({
        ...
        return new FormElementStatus(
            builtStatus.uuid,
            builtStatus.answersToSkip.length < allAnswerConcepts.length,
            undefined,
            answersToSkip
        );
    }
    ...
    _fatherIsAlive(programEncounter, formElement, encounterTypes) {
        return this._parentStatusContains(
            ["Both Alive", "Only Father Alive", "Separated"],
            programEncounter,
            formElement,
            encounterTypes
        );
    ...
    _motherIsAlive(programEnrolment, formElement, encounterTypes) {
        return this._parentStatusContains(
            ["Both Alive", "Only Mother Alive", "Separated"],
            programEnrolment,
            formElement,
            encounterTypes
        );
    ...
    _parentStatusContains(statuses, programEncounter, formElement, encounterTypes) {
        ...
        statusBuilder
            .show()
            .when.latestValueInAllEncounters("Parents' life status")
            .containsAnyAnswerConceptName(...statuses);
    ...
    _applicableForSevereAnemiaCounselling(programEncounter) {
        return new RuleCondition({programEncounter: programEncounter}).when
            ...
            .or.when.valueInLastEncounter(
                "Counselling for Severe Anemia Done",
                RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE
            )
    ...
    _applicableForMalnutritionCounselling(programEncounter) {
        let bmi = lib.C.calculateBMI(
            programEncounter.getObservationValue("Weight"),
            programEncounter.getObservationValue("Height")
        );
        return new RuleCondition({programEncounter: programEncounter})
            .whenItem(bmi)
            .lessThan(18.5)
            .or.when.valueInLastEncounter(
                "Counselling for Malnutrition Done",
                RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE
            )
            .containsAnswerConceptName("No")
            .matches();
    ...
    _applicableForMenstrualDisorderCounselling(programEncounter) {
        return new RuleCondition({programEncounter: programEncounter}).when
            .valueInEncounter("Menstrual disorders")
            .containsAnyAnswerConceptName(
                "Lower abdominal pain",
                "Backache",
                "Leg pain",
    ...
    _applicableForAddictionCounselling(programEncounter) {
        let eitherParentAddicted = new RuleCondition({programEncounter: programEncounter}).when
            .valueInEncounter("Father's Addiction")
            .containsAnyAnswerConceptName("Alcohol", "Tobacco", "Both")
            .or.valueInEncounter("Mother's Addiction")
            .containsAnyAnswerConceptName("Alcohol", "Tobacco", "Both")
            .matches();

        return new RuleCondition({programEncounter: programEncounter})
            .whenItem(eitherParentAddicted)
            .is.truthy.or.when.valueInEncounter("Are friends addicted?")
            .containsAnyAnswerConceptName("Yes")
            .or.when.valueInEncounter("Addiction Details")
            .containsAnyAnswerConceptName("Alcohol", "Tobacco", "Both")
            .or.when.valueInLastEncounter(
                "Counselling for Addiction Done",
                RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE
            )
            .containsAnswerConceptName("No")
            .matches();
    ...
    _applicableForRTICounselling(programEncounter) {
        return new RuleCondition({programEncounter: programEncounter}).when
            .valueInEncounter("Sexually active")
            .containsAnyAnswerConceptName("Yes")
            .or.when.valueInLastEncounter(
                "Counselling for Early Pregnancy & RTI Done",
                RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE
            )
            .containsAnswerConceptName("No")
            .matches();
    ...
    _applicableForRTACounselling(programEncounter) {
        return new RuleCondition({programEncounter: programEncounter}).when
            .valueInEncounter("Drives 2 wheeler")
            .containsAnyAnswerConceptName("Yes")
            .or.when.valueInLastEncounter(
                "Counselling for Road Traffic Accident Done",
                RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE
            )
            .containsAnswerConceptName("No")
            .matches();
    ...
    _applicableForSickleCellAnemiaCounselling(programEncounter) {
        return new RuleCondition({programEncounter: programEncounter}).when
            .valueInLastEncounter("Sickling Test Result", RoutineEncounterHandler.visits.ANNUAL)
            .containsAnyAnswerConceptName("Trait", "Disease")
            .or.when.valueInLastEncounter(
                "Counselling for Sickle Cell Anemia Done",
                RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE
            )
            .containsAnswerConceptName("No")
            .matches();
    ...
    _applicableForDeceasedParentCounselling(programEncounter) {
        return new RuleCondition({programEncounter: programEncounter}).when
            .valueInLastEncounter("Parents' life status", RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE)
            .containsAnyAnswerConceptName("Only Father Alive", "Only Mother Alive", "Both Expired", "Separated")
            .or.when.valueInLastEncounter(
                "Counselling for No Parents / Single Parent Done",
                RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE
            )
            .containsAnswerConceptName("No")
            .matches();
    ...
    _applicableForAddictionFollowup(programEncounter) {
        return (
            this._counsellingForSelfAddictionDoneInPastNVisit(2, programEncounter) ||
    ...
    _counsellingForSelfAddictionDoneInPastNVisit(n, programEncounter) {
        let lastNthEncounter = programEncounter.programEnrolment.findNthLastEncounterOfType(
            programEncounter,
            RoutineEncounterHandler.visits.MONTHLY_OR_MIDLINE,
            n - 1
        );
        ...
        return (
            new RuleCondition({programEncounter: lastNthEncounter}).when
                .valueInEncounter("Counselling for Addiction Done")
                .containsAnswerConceptName("Yes")
                .matches() &&
            new RuleCondition({programEncounter: programEncounter}).when
                .latestValueInPreviousEncounters("Addiction Details")
                .containsAnyAnswerConceptName("Alcohol", "Tobacco", "Both")
                .matches()
        );
    }
    ...
    _schoolAttendanceStatus(programEncounter, formElement, requiredAnswer, encounterTypes) {
        ...
        if (this._isFirstAnnualVisit(programEncounter)) {
            statusBuilder
                .show()
                .when.valueInEnrolment("School going")
                .containsAnswerConceptName(requiredAnswer);
        } else {
            statusBuilder
                .show()
    ...
    _registeredAtSchoolOrBoarding(programEncounter, formElement, encounterTypes) {
        return this._registeredAt(programEncounter, formElement, "School", encounterTypes).or(
            this._registeredAt(programEncounter, formElement, "Boarding", encounterTypes)
        );
    }

    _registeredAtVillageOrBoarding(programEncounter, formElement, encounterTypes) {
        return this._registeredAt(programEncounter, formElement, "Village", encounterTypes).or(
            this._registeredAt(programEncounter, formElement, "Boarding", encounterTypes)
        );
    }
    ...
    _notDroppedOutOrRegisteredAtVillage(programEncounter, formElement, encounterTypes) {
        return this._registeredAtVillage(programEncounter, formElement, encounterTypes).or(
            this._hasBeenComingToSchool(programEncounter, formElement, encounterTypes)
        );
    }

    _villageRegistrationAndDroppedOut(programEncounter, formElement, encounterTypes) {
        return this._registeredAtVillage(programEncounter, formElement, encounterTypes).and(
            this._isDroppedOut(programEncounter, formElement, encounterTypes)
        );
    }

    _schoolRegistrationAndDroppedOut(programEncounter, formElement, encounterTypes) {
        return this._registeredAtSchoolOrBoarding(programEncounter, formElement, encounterTypes).and(
            this._isDroppedOut(programEncounter, formElement, encounterTypes)
        );
    }
    ...
    _notEleventhTwelfthAdolescentRegisteredAtSchoolOrBoardingSchool(programEncounter, formElement, encounterTypes) {
        return new FormElementStatus(
            formElement.uuid,
            !this._isAdolescentEleventhTwelfthStandardAndRegisteredAtSchoolOrBoarding(programEncounter)
        );
    }

    _isAdolescentEleventhTwelfthStandardAndRegisteredAtSchoolOrBoarding(programEncounter) {
        let isAdolescentEleventhTwelfthStandardAndRegisteredAtSchoolOrBoarding = new RuleCondition({
            programEncounter: programEncounter
        }).when
            .latestValueInAllEncounters("Standard")
            .containsAnyAnswerConceptName("11", "12")
            .and.addressType.equalsOneOf("School", "Boarding")
            .matches();
    ...
    _isFirstAnnualVisit(programEncounter) {
        ...
        const firstAnnualEncounter = programEncounter.programEnrolment
            .getEncounters(true)
            .find(encounter => encounter.encounterType.name === "Annual Visit");
        return _.isEmpty(firstAnnualEncounter) || firstAnnualEncounter.uuid === programEncounter.uuid;
    }
