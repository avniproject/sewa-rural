...
class AddictionVulnerabilityViewFilter {
    static exec(programEncounter, formElementGroup, today) {
        return FormElementsStatusHelper
            .getFormElementsStatusesWithoutDefaults(new AddictionVulnerabilityViewFilter(), programEncounter, formElementGroup, today);
    }
    ...
    iftobaccoWhatIsTheType([], statusBuilder) {
        ...
        .latestValueInPreviousEncounters("Addiction Details").containsAnyAnswerConceptName("Tobacco", "Both");
    }

    @WithName("How many pouches do you consume per day?")
    @WithStatusBuilder
    howManyPouchesDoYouConsumePerDay([], statusBuilder) {
        statusBuilder.show().when.valueInEncounter("Type of tobacco").containsAnyAnswerConceptName("Chewable");
    }

    @WithName("How many cigarettes/bidis do you consume per day?")
    @WithStatusBuilder
    howManyCigarettesbidisDoYouConsumePerDay([], statusBuilder) {
        statusBuilder.show().when.valueInEncounter("Type of tobacco").containsAnyAnswerConceptName("Non Chewable");
    }
  ...
  ifYesQuittedThenHowManyDays([], statusBuilder) {
      statusBuilder.show().when.valueInEncounter("Knowing about hazards of tobacco have you quit tobacco/alcohol").containsAnyAnswerConceptName("Yes");
 }
}
